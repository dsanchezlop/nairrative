...
export async function POST(request: NextRequest) {
  try {
    ...
    if (
      !category ||
      !["personaje", "historia", "mundo", "encuentro", "otro"].includes(
        category,
      )
    ) {
      return NextResponse.json(
    ...
    // Llamar a Groq (si llegamos al límite de tokens, cambiamos a llama 3.1)
    ...
    for (const model of MODELS) {
      try {
        ...
        if (parentContent) {
          ...
          messages.push({
            role: "user",
            content: `Continúa o elabora sobre lo anterior siguiendo esta indicación: ${prompt.trim()}`,
          });
        } else {
        ...
        if (isRateLimit) {
          return NextResponse.json(
            {
              error:
                "Límite de la IA alcanzado. Espera un momento e inténtalo de nuevo.",
            },
            { status: 429 },
          );
        }
